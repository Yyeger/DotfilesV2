...
function exportDiagrams(diagrams, format, bar) {
    ...
    if ((0, appliedRender_1.appliedRender)(diagrams[0].parentUri).limitConcurrency()) {
...
function doExportsUnLimited(diagrams, format, bar) {
    ...
    let promises = diagrams.map((diagram, index) => {
        if (!path.isAbsolute(diagram.dir))
            return Promise.reject((0, common_1.localize)(1, null));
        let savePath = (0, tools_1.calculateExportPath)(diagram, format.split(":")[0]);
        (0, tools_1.mkdirsSync)(path.dirname(savePath));
        return (0, exportDiagram_1.exportDiagram)(diagram, format, savePath, bar).promise.then(r => r, err => {
            errors.push(...(0, tools_1.parseError)(err));
            return [];
        });
    });
    return new Promise((resolve, reject) => {
        Promise.all(promises).then(r => {
            ...
            errors.push(...(0, tools_1.parseError)(e));
            reject(errors);
        });
    });
}
...
function doExportsLimited(diagrams, format, concurrency, bar) {
    ...
    for (let i = 0; i < concurrency; i++) {
        ...
        promises.push(diagrams.reduce((prev, diagram, index) => {
            ...
            (0, tools_1.mkdirsSync)(path.dirname(savePath));
            return prev.then(() => {
                return (0, exportDiagram_1.exportDiagram)(diagram, format, savePath, bar).promise;
            }, err => {
                errors.push(...(0, tools_1.parseError)(err));
