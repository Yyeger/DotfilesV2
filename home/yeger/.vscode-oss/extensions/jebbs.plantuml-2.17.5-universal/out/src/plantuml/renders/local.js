...
class LocalRender {
    ...
    createTask(diagram, taskType, savePath, format) {
        if (!config_1.config.java) {
            let pms = Promise.reject((0, common_1.localize)(5, null));
            return { promise: pms };
        }
        if (!fs.existsSync(config_1.config.jar(diagram.parentUri))) {
            let pms = Promise.reject((0, common_1.localize)(6, null, common_1.extensionPath));
            return { promise: pms };
        }
        let processes = [];
        let buffers = [];
        ...
        let pms = [...Array(diagram.pageCount).keys()].reduce((pChain, index) => {
            ...
            return pChain.then(() => {
                ...
                let pms = (0, processWrapper_1.processWrapper)(process, savePath2).then(stdout => buffers.push(stdout), err => Promise.reject({ error: (0, common_1.localize)(10, null, diagram.name, err.error), out: err.out }));
                return pms;
            }, err => {
